...
describe("Near Earth Object Schema", function() {
    describe("NASA NEO Data Helper", function() {
        ...
        it("updates the database with new values ignoring repeated entries", function(done) {
            ...
            let fetchValues = {
                near_earth_objects: near_earth_objects_two
            };

            let promiseReturnValue = Promise.resolve(fetchValues)

            //we expect that the count before is 18
            //we expect that the count after is 20
            //mock out the fetch call used
            let fetchCall = jasmine.createSpy().and.returnValue(promiseReturnValue);
            nasaAPIHelper.fetchAndUpdateNEOs("2001-01-01", "2001-01-01", void 0, fetchCall)
                .then(() => {
